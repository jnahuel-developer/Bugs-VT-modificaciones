...
/* ============================================================
   ...
   Cómo usarlo (resumen):
   ----------------------
   1) Buscar el LocacionID que se quiere usar:
        SELECT LocacionID, Nombre, Numero FROM Locacion ORDER BY Nombre;

   2) Obtener el PasswordHash de la contraseña a utilizar:
        -- Elegir un usuario web que tenga esa contraseña
        SELECT PasswordHash
        FROM AspNetUsers
        WHERE Email = 'email_del_usuario_que_tiene_esa_contraseña';

      Copiar el valor completo devuelto (es una cadena larga) y
      pegr en @DefaultPasswordHash.
...
DECLARE @EmailDomain NVARCHAR(100) = '@LocacionDevCentral.com'; 
...
-- Se deja el PasswordHash obtenido de AspNetUsers para la
-- contraseña 123456.
...
IF @DefaultPasswordHash IS NULL
BEGIN
